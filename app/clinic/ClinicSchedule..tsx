    ...
    useEffect(() => {
        const fetchClinics = async () => {
            try {
                ...
                const response = await axios.get(`http://192.168.8.127:3000/clinic-schedule/upcoming-clinic-schedule/${id}`);
        ...
        return (
            <TouchableOpacity>
                <View className="bg-white m-6 p-3 rounded-lg">
                    <Text className="text-base font-semibold">Schedule {index+1} - {item.clinic.clinicTitle}</Text>
                    <View>
                        <View className="flex-row items-center">
                            ...
                            <View className="w-36 h-36">
                                <Text className="font-semibold text-gray-400 text-base">Date</Text>
                                <Text>{format(new Date(item.date), 'PPPP')}</Text>
                                <Text className="font-semibold text-gray-400 text-base">Time</Text>
                                <Text>{item.time}</Text>
                            </View>
                        </View>
                    </View>
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <View className="" style={{paddingTop: headerHeight }}>
                <Text className="font-semibold text-base p-3 pl-6">Preferred Clinic</Text>
                <View className="bg-white rounded-lg mx-6 py-5">
                    <Text className="pl-3 font-semibold text-base">Choices Pregnancy Center</Text>
                </View>
            </View>
            <Text className="font-bold text-base py-4 pl-6 ">Clinic Appointments for this month</Text>

                <ScrollView style={{ flex: 1 }}>
                    <FlatList data={schedules} renderItem={renderItems} showsVerticalScrollIndicator={false} />
                </ScrollView>
